    ...
    api.post("/admin/sync/push", { password })
      .then((res) => {
        ...
        if (res.data.pushed > 0) {
          toast.success(`Pushed ${res.data.pushed} article${res.data.pushed === 1 ? "" : "s"} to production`);
        } else {
          toast(res.data.message || "Nothing to push.");
        }
      })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-w-lg" data-testid="sync-dialog">
        <DialogHeader>
          ...
          <DialogDescription>
            Copies published articles that exist here but not on your live site. Existing production posts are never modified.
          </DialogDescription>
        </DialogHeader>
          ...
          <>
            ...
            {diff.missing.length === 0 ? (
              <div className="rounded-lg border border-border bg-muted/40 p-5 text-center" data-testid="sync-in-sync">
                ...
                <p className="text-sm">Production already has every published article. You're in sync.</p>
              </div>
            ) : (
              <>
                ...
                <div className="flex justify-end gap-2">
                  ...
                  {!results && (
                    ...
                    >
                      ...
                      Push {diff.missing.length} article{diff.missing.length === 1 ? "" : "s"}
                    </Button>
                  )}
                </div>
              </>
